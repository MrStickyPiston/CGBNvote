...
def plot_votes(con):
    ...
    margin = max(sorted_counts) * 0.0025
    # Adding text labels to the end of the bars
    for index, value in enumerate(sorted_counts):
        plt.text(value + margin, index, str(value), va='center', ha='left')

    plt.savefig('static/results.webp', bbox_inches="tight", dpi=600)
